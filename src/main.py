...
bot = commands.Bot(intents=intents, command_prefix='!', help_command=None, case_insensitive=True, strip_after_prefix=True)
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.invisible) # appear offline initially, whilst bot is setting up

    # EXTENSIONS
    await bot.add_cog(TestCog(bot))

    # ACTIVITY
    activity, avatar = getActivity()
    ...
    global active # TODO bad!
    active = True
    print('Ready.\n')


@bot.listen()
async def on_message(context: Any):
    """Reacts to messages sent to the bot."""

    message: str = context.content.lower()  # removes case sensitivity
    if ((not active) or (context.author == bot.user)):
        return
    ...
    for name in BOT_NAMES: # jerry, ...
        if (name in message):
            ...
            ## BONK
            if ('bonk' in message):
                await bonk(context, message)

            ## SUMMON
            if ('summon' in message):
                await summon(context)
            ...
            if ('wis' in message):
                if (WISDOMS):
                    await context.channel.send(random.choice(WISDOMS).format('<@' + str(context.author.id) + '>'))
                else:
        ...
        ## DANCE
        if ('dance' in message):
            await context.channel.send(random.choice(GIFS['dances']))
            return

        ## GIFS
        for response in GIFS['responses']:
            if (isinstance(response[0], str)): # single prompt
                response[0] = [response[0]]

            for prompt in response[0]:
                if (prompt in message):
                    await sendGIF(context.channel, response[1])
                    return
...
async def bonk(context: Any, target: str):
    """Bonks a user."""

    if (not isUserAdmin(context.author)):

        await reactWithCustomEmote(context, Emotes.BONK.value)

        await sendGIF(context.channel, 'YouHaveNoPowerHere')
        return

    targets = getUsersFromMentions(extractMentionsFromMessage(context.content))
    if (targets):
        ...
        if (bonkRole):
            ...
            if (not toBonk): # only unbonk if no new bonks
                ...
                await sendGIF(context.channel, 'IReleaseYou')
                ...
                # message channel
                await sendGIF(context.channel, random.choice(GIFS['bonks']))
                ...
                if (bonkJail is not None):
                    ...
                    await sendGIF(bonkJail, GIFS['shames'])
                else:
...
async def summon(context: Any):
    """Summons a user or role to the current channel."""

    target = extractSmallestMentionSubset(context.content)
    if (target):
        ...
        await sendGIF(context.channel, random.choice(GIFS['summons']))
...
def extractMentionsFromMessage(message: str) -> dict[str, list[str] | bool]:
...
def extractSmallestMentionSubset(message: str) -> str | bool:
...
async def sendGIF(channel: Any, gifName: str):
...
async def reactWithCustomEmote(context: Any, emote: Emote):
...
def isUserAdmin(user: Any) -> bool:
    """Checks if a user has admin permissions."""

    return user.guild_permissions.administrator

def getUsersFromMentions(mentions: dict[str, list[str] | bool]) -> list[Any]:
